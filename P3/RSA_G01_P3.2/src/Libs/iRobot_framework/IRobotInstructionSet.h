...
// Contains the sensor opcodes that we need to use if we want to query the
// sensor values
...
// Contains the event opcodes that we need to use if we want the iRobot to wait
// one of them
...
// Contains the sensor package opcodes that we need to use if we want to query
// some sensor values at the same time
namespace iRobotSensorPackage {
static const char SENSORS_7_26 = (char)0;  // rango 0-25  ->Actualizar 26 chars
                                           // Cliff, bumper, weels, virtual wall
static const char SENSORS_DIGITAL_BUMPER_CLIFF_WALL_VIRTUALWALL =
    (char)1;  // rango 0-7   ->Actualizar 10   chars (  chars 8,9 sin usar)
              // IR,Buttons,Distance,Angle
static const char SENSORS_IR_BUTTONS_DISTANCE_ANGLE =
    (char)2;  // rango 10-15 ->Actualizar 6    chars
static const char SENSORS_BATERY =
    (char)3;  // rango 16-25 ->Actualizar 10   chars
static const char SENSORS_ANALOG_CLIFF_WALL_VIRTUALWALL =
    (char)4;  // rango 27-39 ->Actualizar 14   chars
...
class iRobotInstructionSet {
   private:
    //************************************
    // Method:    intHightByte
    // Returns:   char						[out]	Hight byte of
    // the number Parameter: short num					[in]
    // The number
    //************************************
   ...
   public:
    //************************************
    // Method:    sensorReturn
    // Returns:   int						[out]	How many bytes generates the sensor
    // as response Parameter: char sensor				[in] The
    // sensor id
    //************************************
